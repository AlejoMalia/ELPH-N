...
def lee_imagen(im, calib, fid_nom):
    ...
    return corrige_silueta(desdistorsiona(P, calib)), diametros(b, im.shape)
...
def veredicto(origen, destino, suelos, impostores, suelos_inv=None):
    ...
    return dict(suelo=su, umbral=umb, delta=DELTA, dispersion_suelo=disp_rel,
                ...
                pasa_contrato=bool(A > 1 and bajo == 0) if impostores else None,
...
def main():
    ...
    if modo == 'calibrar':
        ...
        print(f"  IMPOSTOR   **{v['impostor']:.5f}**   (32 direcciones equivocadas)")
